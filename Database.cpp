...
void StyleDatabase::open()
{
	...
	if (sqlite3_open("AIsql.db3", &db) == SQLITE_OK)
	{
		...
		cerr << "SQLite Database failed to open" << endl;
	}
...
{
	insert("INSERT INTO HMMTokenPaths (SentenceID,StyleID,CurrentToken,NextToken,PreviousToken) VALUES(" + to_string(SentID) + "," + to_string(StyleID) + "," + to_string(CurToken) + "," + to_string(NextToken) + ","+to_string(PrevToken)+");");
}
...
void StyleDatabase::CreateDatabase()
{
	...
	if (ret_code != 0)
	{
		...
		for (int i = 0; i < tables.size();i++)
		{
			...
			char dropStr[500];
			snprintf(dropStr, 500, "DROP TABLE %s", tables[i].c_str());
